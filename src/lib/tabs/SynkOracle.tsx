...
import { Share2, Download, RefreshCw, Shield } from "lucide-react";
import { useSYNK } from "../Store";
import AespaFanlight from "./AespaFanlight";
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-12 md:gap-20">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center md:items-end gap-10 border-b border-white/10 pb-16 relative">
          <div className="flex flex-col items-center md:items-start gap-4">
             ...
             <h1 className="high-fashion-header text-5xl sm:text-7xl md:text-9xl scale-y-90 origin-bottom leading-none px-4 md:px-0">ORACLE</h1>
          </div>
        ...
        {phase === 'prism' ? (
            ...
            <div className="max-w-xl text-center flex flex-col items-center gap-8 px-6">
              ...
              <div className="flex flex-col gap-16 items-center">
                 <p className="text-[10px] md:text-[12px] text-white/40 tracking-[0.4em] uppercase font-light text-center">
                    Align your resonance. Hold the fanlight to manifest.
                 </p>
                 
                 <AespaFanlight onShatter={handleShatter} />
              </div>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-16 md:gap-24 animate-in fade-in zoom-in duration-700">
             ...
             {/* THE PHOTOCARD */}
             <div className="relative w-full max-w-[420px] min-h-[580px] md:min-h-[640px] [perspective:1000px] group transition-transform duration-500 hover:[transform:rotateX(5deg)_rotateY(5deg)] active:scale-95 cursor-pointer">
                <div className="relative w-full h-full bg-[#0a0a0a] border border-white/20 p-6 flex flex-col gap-6 shadow-[0_0_80px_rgba(179,136,255,0.1)] overflow-hidden rounded-[4px]">
                   ...
                   <div className="flex justify-between items-start border-b border-white/10 pb-4">
                      <div className="flex flex-col">
                         <span className="text-[10px] uppercase tracking-widest text-synk-lavender font-bold">SYNK PHOTOCARD</span>
                         <span className="text-[8px] font-mono text-white/20 uppercase">No. {Math.floor(Math.random() * 9999).toString().padStart(4, '0')}</span>
                      </div>
                      <span className={cn("text-[10px] font-mono font-bold tracking-tighter uppercase", getStatusColor(result?.status))}>
                         {result?.status?.replace(/_/g, ' ')}
                      </span>
                   </div>

                   {/* Photocard Image Section */}
                   <div className="relative w-full aspect-[4/5] overflow-hidden bg-black/40 border border-white/5 mt-4">
                      <img 
                   ...
                   {/* Main Content Area */}
                   <div className="flex-1 flex flex-col gap-3 justify-center py-2 overflow-hidden min-h-0">
                      <span className="text-[8px] md:text-[9px] uppercase tracking-[0.4em] text-synk-pink border-l-2 border-synk-pink pl-3 shrink-0">PRIMARY MANIFEST</span>
                      <div className="flex flex-col gap-3 overflow-y-auto no-scrollbar pr-1 flex-1 min-h-0">
                        <h2 className={cn(
                          "font-zh-serif leading-[1.1] text-white font-bold transition-all",
                          result?.kr && result.kr.length > 30 ? "text-xl md:text-2xl" : 
                          result?.kr && result.kr.length > 15 ? "text-2xl md:text-3xl" : 
                        ...
                        <div className="space-y-2 pt-2 border-t border-white/5">
                           <p className="text-[10px] md:text-[11px] font-serif italic text-white/40 leading-relaxed uppercase tracking-wide">
                              "{result?.en}"
                           </p>
                           <p className="font-zh-serif text-[11px] md:text-[12px] text-synk-lavender/60 leading-relaxed italic">
                              「{result?.tc}」
                           </p>
                        </div>
                      </div>
                   ...
                   <div className="mt-auto space-y-4 shrink-0">
                      <div className="bg-white/[0.03] p-3 border border-white/5 rounded-sm">
                         <span className="text-[7px] uppercase tracking-[0.4em] text-white/30 block mb-1">SYNC ADVICE</span>
                         <div className="max-h-20 overflow-y-auto no-scrollbar">
                           <p className="text-[10px] font-zh-serif text-white/80 leading-relaxed italic">
                              {result?.advice}
                           </p>
                         </div>
                      </div>
                      ...
                      <div className="flex justify-between items-end gap-4">
                         <div className="flex flex-col">
                            <span className="text-[7px] uppercase tracking-widest text-white/20">SOURCE ARCHIVE</span>
                            <span className="text-[11px] font-serif uppercase tracking-widest text-white/60">{result?.track}</span>
                         </div>
                         <div className="text-right">
                            <span className="text-[7px] uppercase tracking-widest text-white/20">IDENTIFIER</span>
                            <span className="text-[11px] font-mono uppercase text-white/60 block leading-none">{result?.keyword}</span>
                         </div>
                      </div>
                   </div>
